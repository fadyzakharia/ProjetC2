...
public class deleteCard extends HttpServlet {
    ...
    protected void service(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        String card_id = req.getParameter("id");
	...
	ClientCard card = cardfacade.find(Long.valueOf(card_id));
        ...
        if (!cardtrans.isEmpty()){
            for(Transactions t : cardtrans){
                transfacade.remove(t);
            }
        }
                
        cardfacade.remove(card);
        
        Long account_id = card.getAccountId().getId();
        
        List<ClientCard> cardsList = cardfacade.findByAccountId(account_id.toString());
